...
- 本目录的 `src/` 目前是**较新的那一份**。历史上 web 版曾是主副本，现在反过来：**不要再用 `wenVedio_web/src` 覆盖本目录 `src`**，那会把界面重做、AI 自动配置、防重复配置这些改动全部冲掉。
- 要同步的话方向是反向的：

  ```bash
  # 以客户端为准，覆盖 web 端（覆盖后 web 端即可使用新界面与新功能）
  cp -r wenVedio_Client/src/. wenVedio_web/src/
  ```

  桌面专属分支由 `window.wenvedioDesktop` 判断，web 端不受影响。
- 两份 `src/` 是重复副本，改完记得同步，否则会再次走偏。
