...
├── store.js           定义、版本、运行记录的持久化
├── engine.js          引擎：拓扑推进、错误策略、断点续跑、取消/暂停
...
├── canvas.js          SVG 画布
├── editor.js          列表 / 编辑器 / 运行记录 / 环境面板
└── workflow.css
```

工作流统一存成 JSON（`data/config/workflows.json`），运行记录在 `data/config/workflow-runs.json`（上限 200 条，超出丢最旧的已结束记录）。

**这一批没做的**（按之前商定的收窄）：循环节点、条件分支、子工作流、模板市场、版本对比、定时与 Webhook 触发，以及「图片生成页里选择工作流生成」的入口。
